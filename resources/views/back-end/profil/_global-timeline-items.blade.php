...
<div class="profil-timeline-item profil-timeline-item--global">
    <div class="profil-timeline-dot profil-timeline-dot--{{ $log->action }}"></div>
    <div class="profil-timeline-content">
        <div class="profil-timeline-icon">
            @switch($log->model_label)
                @case('Article')
                    <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                    @break
                @case('Evenement')
                    <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/></svg>
                    @break
                @case('Dossier')
        ...
        <div class="profil-timeline-text">
            @if($log->user)
            <span class="profil-timeline-user">
                @php
                ...
                @endphp
                <span class="profil-timeline-user-avatar" style="background: {{ $log->user->avatar ? 'transparent' : $avatarBg }};">
                    @if($log->user->avatar)
                ...
                <a href="{{ route('admin.profil.show', $log->user) }}">{{ $log->user->name }}</a>
            </span>
            @endif
            <p class="profil-timeline-desc">{{ $log->description }}</p>
            <div class="profil-timeline-meta">
                <span>{{ $log->created_at->diffForHumans() }}</span>
                @if($log->ip_address)
                <span class="profil-timeline-ip">{{ $log->ip_address }}</span>
                @endif
            </div>
        </div>
    </div>
