...
@section('breadcrumb', 'FAQ / Créer')

@section('content')
<div class="content-header">
    <div>
        <h1 class="content-title">Nouvelle FAQ</h1>
        <p class="content-subtitle">Ajoutez une nouvelle question/réponse</p>
    </div>
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <select id="faqable_type" name="faqable_type" class="form-select">
                    ...
                    <option value="App\Models\Dossier" {{ old('faqable_type') == 'App\Models\Dossier' ? 'selected' : '' }}>Dossier</option>
                </select>
            ...
            <div class="form-group" id="dossiers-group" style="display:none;">
                ...
                <select id="faqable_id_dossier" name="" class="form-select">
                    ...
                    @foreach($dossiers as $dossier)
                        <option value="{{ $dossier->id }}" {{ old('faqable_id') == $dossier->id && old('faqable_type') == 'App\Models\Dossier' ? 'selected' : '' }}>{{ $dossier->title }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group full-width">
                ...
                <textarea id="question" name="question" class="form-textarea" rows="3" required placeholder="Saisissez la question">{{ old('question') }}</textarea>
                @error('question') <span class="form-error">{{ $message }}</span> @enderror
            </div>

            <div class="form-group full-width">
                <label for="answer">Réponse <span class="required">*</span></label>
                <textarea id="answer" name="answer" class="form-textarea wysiwyg" rows="5" required placeholder="Saisissez la réponse">{{ old('answer') }}</textarea>
